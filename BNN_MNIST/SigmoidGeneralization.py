...
if flag_nnsghmc==False:
    ...
    for total_idx in range(total_run):
        ...
        num_chain=20
        MLP_mnist=BNN(dim=784,hidden=40,layer_num=3,dim_out=10,act_func='Sigmoid')
        ...
        if flag_SGLD==True:
            ...
            eps=0.15/data_N # this is the learning rate, in original paper this = eps_true**2=gamma/data_N (gamma is the per sampler lr)
        if flag_SGLD==False:
        ...
        if flag_SGLD==True:
            ...
            torch.save(tensor_state_list_sgld,'./Sigmoid_Generalization_Long_Run/Test_long_run_sgld_%s'%(total_idx+1))
            ...
            np.savetxt('./Sigmoid_Generalization_Long_Run/Test_long_run_sgld_%s_result'%(total_idx+1),Result)
        else:
            tensor_state_list_sghmc=torch.stack(tuple(state_list_sghmc),dim=0)
            torch.save(tensor_state_list_sghmc,'./Sigmoid_Generalization_Long_Run/Test_long_run_sghmc_%s'%(total_idx+1))
            ...
            Result=np.concatenate(([Acc],[NLL.cpu().data.numpy()],[ed]))
            np.savetxt('./Sigmoid_Generalization_Long_Run/Test_long_run_sghmc_%s_result'%(total_idx+1),Result)
else:
    for total_idx in range(total_run):
        ...
        D=parallel_D_eff(total_dim,D_MLP,MLP_mnist,num_chain,clamp_min=0.,clamp_max=10000,dim_pen=1.,dim_pen_p=1,dim_pen_g=1,sqrt=False)
        ...
        eps=float(np.sqrt(0.085/data_N))
        eps2=float(np.sqrt(0.18/data_N))
        sigma=1.
        const_Q=0.
        ...
        torch.save(tensor_state_list_nnsghmc,'./Sigmoid_Generalization_Long_Run/Test_long_run_nnsghmc_%s_broad_0.2_fd_0.18'%(total_idx+1))
        ...
        Result=np.concatenate(([Acc],[NLL.cpu().data.numpy()],[ed]))
        np.savetxt('./Sigmoid_Generalization_Long_Run/Test_long_run_nnsghmc_%s_result_broad_0.2_fd_0.18'%(total_idx+1),Result)
